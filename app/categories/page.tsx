...
export default async function CategoriesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="mb-3 font-medium">Add category</div>
        <CategoryForm />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="card">
          ...
          <ul className="text-sm text-neutral-700 list-disc pl-5">
            {income.map(c => <li key={c.id}>{c.name}</li>)}
            {!income.length && <li className="text-neutral-500">None</li>}
          </ul>
        </div>
        <div className="card">
          <div className="mb-2 font-medium">Expenses</div>
          <ul className="text-sm text-neutral-700 list-disc pl-5">
            {expense.map(c => <li key={c.id}>{c.name}</li>)}
            {!expense.length && <li className="text-neutral-500">None</li>}
          </ul>
        </div>
      </div>
    </div>
